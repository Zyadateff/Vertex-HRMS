        ...
        private async Task BuildCacheInBackgroundAsync(CancellationToken token)
        {
            ...
            try
            {
                ...
                foreach (var emp in employees)
                {
                    ...
                    {
                        _logger.LogWarning("⚠ No valid encoding for Employee {Id} at {Path}", emp.EmployeeId, processedPath);
                    }
                }
                ...
                if (snapshot.Count == 0)
                {
                    ...
                    try
                    {
                        ...
                        foreach (var emp in employees)
                        {
                            ...
                            {
                                _logger.LogWarning($"⚠ Failed to encode image for EmpId={emp.EmployeeId}, File={relativePath}");
                            }
                        }
